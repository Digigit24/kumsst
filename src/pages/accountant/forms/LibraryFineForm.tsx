...
import type { LibraryFine } from '../../../types/accountant.types';

interface LibraryFineFormProps {
    libraryFine: LibraryFine | null;
    onSubmit: (data: any) => void;
    onCancel: () => void;
    isSubmitting?: boolean;
}

export function LibraryFineForm({ libraryFine, onSubmit, onCancel, isSubmitting = false }: LibraryFineFormProps) {
    ...
    const { data: bookIssuesData, isLoading: bookIssuesLoading } = useBookIssues({
        ...
        // status: 'issued,overdue', // Fetch all issues so we can fine for returned books (damage etc)
        page: 1,
        page_size: DROPDOWN_PAGE_SIZE,
    });
    ...
    useEffect(() => {
        if (libraryFine) {
            setFormData({
                ...
                book_issue: libraryFine.book_issue,
                fine_amount: Number(libraryFine.amount || libraryFine.fine_amount),
    ...
    const memberOptions =
        membersData?.results?.map((member: any) => ({
            ...
            label: `${member.user_name} (${member.member_id})`,
        })) || [];
        ...
        <form onSubmit={handleSubmit} className="space-y-6 p-6">
            <Card className="p-6 space-y-4">
                ...
                <div className="space-y-2">
                    ...
                    <SearchableSelect
                        ...
                        placeholder={formData.member ? "Select book issue..." : "Select a member first"}
                        emptyText="No active book issues found."
                ...
                <div className="space-y-2">
                    ...
                    <Input
                        ...
                        onChange={(e) => handleChange('fine_amount', parseFloat(e.target.value) || 0)}
                        required
                    />
                </div>
            ...
            <div className="flex gap-3 justify-end">
                ...
                <Button type="submit" loading={isSubmitting}>
                    {libraryFine ? 'Update' : 'Create'} Library Fine
                </Button>
            </div>
        </form>
    );
