...
import { useFeeClassDrillDown } from '@/hooks/useFeeDrillDown';
import {
    AlertCircle,
    ArrowRight,
    ChevronLeft,
...
export const FeeClassDrillDown: React.FC = () => {
    const { yearId, programId, classId } = useParams<{ yearId: string; programId: string; classId: string }>();
    const navigate = useNavigate();
    const [filters, setFilters] = useState({});
    const { data, isLoading, error, refetch } = useFeeClassDrillDown(classId ? parseInt(classId) : null, filters);
    ...
    return (
        <div className="space-y-8 p-6 animate-fade-in bg-slate-50/50 min-h-screen">
            ...
            {/* Hero Header */}
            <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-rose-600 via-orange-600 to-amber-600 shadow-2xl">
                        ...
                        <div className="space-y-2">
                            <div className="flex items-center gap-3">
                                <Button
                                    variant="ghost"
                                    size="icon"
                                    onClick={() => navigate(`/finance/drilldown/${yearId}/program/${programId}`)}
                            ...
                            <p className="text-orange-100 text-lg pl-14">
                                Fee Type Collection Analysis
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <Card className="border-none shadow-lg bg-white/80 backdrop-blur-sm overflow-hidden">
                <CardHeader className="border-b bg-muted/30 px-6 py-5">
                    <CardTitle className="text-xl font-bold text-gray-800">Fee Type Breakdown</CardTitle>
                </CardHeader>
                        ...
                        <Table>
                            <TableHeader className="bg-muted/30">
                                <TableRow className="hover:bg-transparent border-none">
                                    <TableHead className="pl-6 h-12">Fee Type</TableHead>
                                    ...
                                    <TableHead className="text-center h-12">Progress</TableHead>
                                    <TableHead className="text-right pr-6 h-12">Action</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                {data?.fee_type_breakdown.map((feeType, index) => (
                                    <TableRow
                                        key={feeType.fee_type_id}
                                        className={`cursor-pointer transition-colors border-b border-muted/50 last:border-0 hover:bg-orange-50/50 ${index % 2 === 0 ? 'bg-white' : 'bg-slate-50/30'}`}
                                        onClick={() => navigate(`/finance/drilldown/${yearId}/program/${programId}/class/${classId}/fee-type/${feeType.fee_type_id}`)}
                                    >
                                        <TableCell className="font-medium pl-6 py-4">
                                            <div className="flex items-center gap-3">
                                                <div className="h-10 w-10 rounded-lg bg-orange-100 text-orange-600 flex items-center justify-center font-bold">
                                                    <FileText className="h-5 w-5" />
                                                </div>
                                                <span className="text-base text-gray-900">{feeType.fee_type_name}</span>
                                            </div>
                                        </TableCell>
                                        <TableCell className="text-right font-medium">
                                            {formatCurrency(feeType.total_due || 0)}
                                        </TableCell>
                                        <TableCell className="text-right text-green-600 font-medium">
                                            {formatCurrency(feeType.total_collected || 0)}
                                        </TableCell>
                                        <TableCell className="text-right text-red-600 font-medium">
                                            {formatCurrency(feeType.total_outstanding || 0)}
                                        </TableCell>
                                        <TableCell className="text-center w-[15%]">
                                            <div className="flex flex-col gap-1 items-center">
                                                <div className="w-full bg-gray-100 rounded-full h-2 overflow-hidden">
                                                    <div
                                                        className={`h-full rounded-full transition-all duration-500 ${(feeType.collection_rate || 0) >= 90 ? 'bg-green-500' :
                                                            (feeType.collection_rate || 0) >= 75 ? 'bg-yellow-500' : 'bg-red-500'
                                                            }`}
                                                        style={{ width: `${feeType.collection_rate || 0}%` }}
                                                    />
                                                </div>
                                                <span className="text-xs font-semibold">{(feeType.collection_rate || 0).toFixed(1)}%</span>
                                            </div>
                                        </TableCell>
                                        <TableCell className="text-right pr-6">
                                            <Button
                                                ...
                                                onClick={(e) => {
                                                    e.stopPropagation();
                                                    navigate(`/finance/drilldown/${yearId}/program/${programId}/class/${classId}/fee-type/${feeType.fee_type_id}`);
                                                }}
                                            >
                                                Details
